...
async function editIndex(
  ...
  try {
    ...
    if (serverIndex === -1) {
      console.error(`Bedrock server with id ${id} not found`);
      return false;
    }
    ...
    console.error('Error editing bedrock server index:', error);
    return false;
  }
}
...
ipcMain.handle(
  ...
  async (
    _e,
    { id, updates }: { id: string; updates: Partial<BedrockServer> },
  ) => {
    return editIndex(id, updates);
  },
);

ipcMain.handle('bedrock:get', async (_e, { id }: { id: string }) => {
  const list = await loadIndex();
...
ipcMain.handle('bedrock:getRunningStatus', async () => {
...
ipcMain.handle(
  ...
  async (_e, { id }: { id: string }) => {
    ...
    const propsPath = path.join(srv.dir, 'server.properties');
    const parsed = await parseServerProperties(propsPath);
    return parsed;
  },
);

ipcMain.handle(
  'bedrock:saveServerProperties',
  async (
    _e,
